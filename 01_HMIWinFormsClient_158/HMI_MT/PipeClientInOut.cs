...
namespace HMI_MT
{
   public class PipeClientInOut
   {
      ...
      /// <summary>
      /// ссылка на массив для принимаемых пакетов, пакетов передаваемых на обработку 
      /// объекту класса ServerDataForExchange
      /// </summary>
      ...
      public void Start(int intervalForConnect)
      {
         return;
      ...
      private void WaitForPipeConnect(int intervalForConnect)
      {
         ...
         try
         {
            ...
            pipeClientInOut.ReadMode = PipeTransmissionMode.Message;
      ...
      {
         TraceSourceLib.TraceSourceDiagMes.WriteDiagnosticMSG(TraceEventType.Warning, 0, DateTime.Now.ToString() + " (0) : Точка = 7 = (PipeClientInOut.cs::AsyncPipeBeginReadLenData()) ");
         try
         {
            ...
            TraceSourceLib.TraceSourceDiagMes.WriteDiagnosticMSG(TraceEventType.Warning, 0, DateTime.Now.ToString() + " (0) : Точка = 7 -> 8 = (PipeClientInOut.cs::AsyncPipeBeginReadLenData()) ");
         }
         catch (Exception ex)
         {
      ...
      /// <summary>
      /// буфер для приема пакетов ще pipe-сервера
      /// </summary>
      ...
      {
         TraceSourceLib.TraceSourceDiagMes.WriteDiagnosticMSG(TraceEventType.Warning, 1, DateTime.Now.ToString() + " (1) : Точка = 7 = (PipeClientInOut.cs::AsyncPipeBeginReadLenData()) : длина пакета в DataServer = " + pq.Length);
