...
{
    public partial class frmSirius_OZZ : frmBMRZbase, IDeviceForm
   {
       /// <summary>
      ...
      #region Конструкторы
      public frmSirius_OZZ( )
      {
         InitializeComponent( );
      }
      public frmSirius_OZZ(int iFC, int iIDDev, /*int inumLoc,*/ string fXML, string ftagsxmldescript)
          : base(iFC, iIDDev, fXML, ftagsxmldescript)
      {
         InitializeComponent( );
      ...
      private void frmSirius_OZZ_Load(object sender, EventArgs e)
      {
         ...
         path2PrgDevCFG = AppDomain.CurrentDomain.BaseDirectory + Path.DirectorySeparatorChar + "Project\\Configuration\\0#DataServer\\Sources\\MOA_ECU" + Path.DirectorySeparatorChar + "PrgDevCFG.cdp";
         
         XDocument xdoc = XDocument.Load(path2PrgDevCFG);
         IEnumerable<XElement> xes = xdoc.Descendants("SourceECU");
         var xe = (from nn in
                      (from n in xes
                       where n.Attribute("numFC").Value == StrFC
                       select n).Descendants("Device")
                   where nn.Element("DescDev").Element("NumDev").Value == IIDDev.ToString()  //( IIDDev - ( 256 * IFC ) )
                   select nn).First();

         string numdev = xe.Attribute("objectGUID").Value;
        xe = xe.Element("DescDev");
        
        //path2DeviceCFG = AppDomain.CurrentDomain.BaseDirectory + Path.DirectorySeparatorChar + "Project" + Path.DirectorySeparatorChar + xe.Element("nameR").Value + Path.DirectorySeparatorChar + "Device.cfg";        
        
        path2DeviceCFG = nfXMLConfig + Path.DirectorySeparatorChar + "Device.cfg";        
         ...
         //path2FrmDev = AppDomain.CurrentDomain.BaseDirectory + Path.DirectorySeparatorChar + xe.Element("nameR").Value + Path.DirectorySeparatorChar + numdev + "@frm" + xe.Element("nameELowLevel").Value + ".xml";
        
         path2FrmDev = fileFrmTagsDescript;
      ...
      private void frmSirius_OZZ_FormClosing(object sender, FormClosingEventArgs e)
      {
      ...
      public void InitInterfaceElementsClick()
      {
          ...
          //tabPageInfo.Enter +=new EventHandler(tabPageInfo_Enter);
          ...
          this.FormClosing += new FormClosingEventHandler(frmSirius_OZZ_FormClosing); 
      ...
      public void ActivateTabPage(string typetabpage)
      {
          try
          {
              ...
              {
                  case "Авария": 
                      tc_Main.SelectedTab = tabpageSrabat;  
                      break;
                  default:
      ...
      void tabpageControl_Enter(object sender, EventArgs e)
      {
         ...
         if (arrVars.Count != 0)
         {
             tpCurrent = HMI_MT_Settings.HMI_Settings.SetTagsSubscribe4TPCurrent(tabpageControl);
             return;
         }

         PrepareTabPagesForGroup(tp_this.Text, tp_this, ref arrVars, null/*pnlTPControl*/, false );
      ...
      private void PrepareAdditionalFLP(Control.ControlCollection cntrlCC)
      {
         ...
         foreach (XElement flpframe in flpframes)
         {
            ...
            // отображаем
            PlaceVisElemOnForm("", flpframe.Attribute("MTFLPNameR").Value, arrf, false);
         }
      }
      #endregion
      ...
      void tabpageSrabat_Enter(object sender, EventArgs e)
      {
         ...
         */
         ...
         {
             tpCurrent = HMI_MT_Settings.HMI_Settings.SetTagsSubscribe4TPCurrent(tabpageSrabat);
             return;
         }


         PrepareTabPagesForGroup(tp_this.Text, tp_this, ref arrVars, pnlTPSpab, false);
         slTPtoArrVars[tp_this.Text] = arrVars; // ref не отрабатывает (?)
      ...
      private void lstvAvar_ItemActivate(object sender, EventArgs e)
      {
          try
          {
              ...
              IRequestData req = HMI_MT_Settings.HMI_Settings.CONFIGURATION.GetData(0, (uint)numdevfc, "ArhivAvariBlockData", arparam, id_block);
      ...
      private void AvarBD()
      {
          try
          {
              ...
              for (int curRow = 0; curRow < dtA.Rows.Count; curRow++)
              {
                  ...
                  ListViewItem li = new ListViewItem(string.Format("{0} - {1}", CommonUtils.CommonUtils.GetTimeInMTRACustomFormat(t), dtA.Rows[curRow]["Arg1"].ToString()));
      ...
      void tabpageConfig_Enter(object sender, EventArgs e)
      {
         ...
         if (arrVars.Count != 0)
         {
             tpCurrent = HMI_MT_Settings.HMI_Settings.SetTagsSubscribe4TPCurrent(tabpageConfig); 
             return;
         }

         PrepareTabPagesForGroup(tp_this.Text, tp_this, ref arrVars, pnlTPConfig, true);
         slTPtoArrVars[tp_this.Text] = arrVars; // ref не отрабатывает (?)
      ...
      void btnWriteConfig_Click(object sender, EventArgs e)
      {
         ...
         //System.Buffer.BlockCopy((byte[])slLocal[60200], 16, memX, 0, lenpack - 16);//6 <-> 6+8 - 8 байт - два времени (см. протокол)
         ...
         //{
         //   if (UstavkiControls[i] is ctlLabelTextbox)
         //   {
         //      ultb = (ctlLabelTextbox)UstavkiControls[i];
         //      if (ultb.isChange)
         //      {
         ...
         //               ainmemX = (ultb.addrLinkVar - 60205) * 2;//baseAdr
         ...
         //      }
         //   }
         //   else if (UstavkiControls[i] is ComboBoxVar)
         //   {
         //      cbTmp = (ComboBoxVar)UstavkiControls[i];
         //      if (cbTmp.isChange)
         //      {
         //         isUstChange = true;
         ...
         //         Array.Reverse(aTmp2);
         //         // запоминаем изменения
         //         ainmemX = (a - 60205) * 2;//60200 <> 60204
         //         Buffer.BlockCopy(aTmp2, 0, memX, (int)ainmemX, 2);
         //      }
         //   }
         //   else if (UstavkiControls[i] is CheckBoxVar)
         //   {
         //      chbTmp = (CheckBoxVar)UstavkiControls[i];
         //      if (chbTmp.isChange)
         //      {
         //         isUstChange = true;
      ...
      private void UstavBD()
      {
          ...
          for (int curRow = 0; curRow < dtU.Rows.Count; curRow++)
          {
              ...
              ListViewItem li = new ListViewItem(string.Format("{0} - {1}", CommonUtils.CommonUtils.GetTimeInMTRACustomFormat(t), dtU.Rows[curRow]["Arg1"].ToString()));
      ...
      void CreateTabPanel()
      {
         ...
         */
         ...
         tc_Main.Controls.Add( new DataBaseFilesLibrary.TabPageDbFile( (uint)( 256 * IFC + IIDDev ) ) );
      }
      #endregion
      ...
      private void button1_Click(object sender, EventArgs e)
      {
         ...
         //StringBuilder cmd2 = new StringBuilder();
         //cmd2.Append((sender as Button).Text.ToString() + "\0");
         //byte[] scmd2 = Encoding.UTF8.GetBytes(cmd2.ToString());
         //Buffer.BlockCopy(scmd2, 0, memXOut, 0, scmd2.Length);

         //if (parent.newKB.ExecuteCommand(IFC, IIDDev, "CTO", String.Empty, memXOut, parent.toolStripProgressBar1, parent.statusStrip1, parent))
         //   parent.WriteEventToLog(35, "Команда \"CTO\" ушла в сеть. Устройство - " + IIDDev.ToString(), this.Name, false);
      }
      #endregion

      /// <summary>
      /// Команда Пуск цикла опроса всех фидеров
      /// </summary>
      /// <param Name="sender"></param>
      /// <param Name="e"></param>
      private void button25_Click(object sender, EventArgs e)
      {
         //if (parent.newKB.ExecuteCommand(IFC, IIDDev, "RUN", String.Empty, null, parent.toolStripProgressBar1, parent.statusStrip1, parent))
         //   parent.WriteEventToLog(35, "Команда \"RUN\" ушла в сеть. Устройство - " + IIDDev.ToString(), this.Name, false);
      }
      ...
      {
         //if (parent.newKB.ExecuteCommand(IFC, IIDDev, "RST", String.Empty, null, parent.toolStripProgressBar1, parent.statusStrip1, parent))
         //   parent.WriteEventToLog(35, "Команда \"RST\" ушла в сеть. Устройство - " + IIDDev.ToString(), this.Name, false);
      }
