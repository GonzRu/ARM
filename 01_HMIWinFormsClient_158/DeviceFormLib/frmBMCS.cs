...
namespace DeviceFormLib
{
    public partial class frmBMCS : Form, IDeviceForm
	{
        ...
        private bool tab1, tab2 = false, tab3 = false;
        ...
        public frmBMCS(/*Form MainForm linkMainForm,*/ int iFC, int iIDDev, /*int inumLoc,*/ string fXML, string ftagsxmldescript)
		{
            ...
            XElement xedev = HMI_Settings.CONFIGURATION.GetDeviceXMLDescription(0, "MOA_ECU", iFC * 256 + iIDDev);
            xedev = xedev.Element("DescDev");   // подправили

            //string TypeName = xedev.Element("TypeName").Value;
            string nameR = xedev.Element("nameR").Value;
            //string nameELowLevel = xedev.Element("nameELowLevel").Value;
            //string nameEHighLevel = xedev.Element("nameEHighLevel").Value;
            Text = nameR + " ( ид.№ " + iIDDev.ToString() + " )" + xedev.Element("DescDev").Value;
        ...
        private void frmBMCS_Load( object sender, EventArgs e )
        {
            try
            {
                ...
                dtpEndDateConfig.Value = new DateTime( DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 23, 59,
                                                       59, 999 );
                dtpEndTimeConfig.Value = new DateTime( DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 23, 59,
                                                       59, 999 );
                dtpStartDateConfig.Value = new DateTime( DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 0, 0,
                                                         0, 0 );

                ts = new TimeSpan( 1, 0, 0, 0 );
                dtpStartDateConfig.Value = dtpStartDateConfig.Value - ts;
                dtpStartTimeConfig.Value = new DateTime( DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 0, 0,
                                                         0, 0 );
                ...
                tabControl1.Controls.Add( new DataBaseFilesLibrary.TabPageDbFile( (uint)( iFC * 256 + iIDDev ) ) );

                //скрыто по просьбе В.И. в связи с сложностью реализации битовых сигналов из перечисления
                tabCntrl.Controls.Remove( ( tbpUst_2 ) );
            }
            catch ( Exception ex )
            {
            ...
            // tbBMCS_Indication_Enter(sender, e);

            SelectTabPage( tbBMCS_Indication );
        }
        ...
        public void InitInterfaceElementsClick()
        {
            ...
            FormClosing += frmBMCS_FormClosing;
        }
        ...
        private void SelectTabPage( TabPage page )
        {
            Debug.Assert( page != null, "page != null" );
            
            timerInd.Stop();
            if ( page.Name == tbBMCS_Indication.Name )
            {
                timerInd.Start();

                if ( !tab1 )
                {
                    tbBMCS_Indication_Enter();
                    tab1 = true;
                }
                // подписываемся на теги
                tpCurrent = HMI_Settings.SetTagsSubscribe4TPCurrent( tbBMCS_Indication );
            }
            else if ( page.Name == tbBMCS_Store.Name )
            {
                if ( !tab2 )
                {
                    tbBMCS_Store_Enter();
                ...
                // подписываемся на теги
                tpCurrent = HMI_Settings.SetTagsSubscribe4TPCurrent( tbBMCS_Store );
            }
            else if ( page.Name == tbBMCS_Config.Name )
            {
                if ( !tab3 )
                {
                    tbBMCS_Config_Enter();
                ...
                // подписываемся на теги
                tpCurrent = HMI_Settings.SetTagsSubscribe4TPCurrent( tbBMCS_Config );
            }
        }
		...
		private void tbBMCS_Indication_Enter()
		{
        ...
        {
            var arrVar = new ArrayList();

            // вход 1
            //efv = new FormulaEval(parent.KB, "0(" + iFC + "." + iIDDev + ".5.112.0100)", "0", "Вход 1", "", TypeOfTag.Discret, "");
            var efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.0100)", "0", "Вход 1", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_1.LinkSetText_Ind;
            ciIN_1.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.0100)", "0", "Вход 1", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_1.LinkSetText_IndFlash;
            ciIN_1.SetSubscribeLink2Tag(efv.LinkVariableNewDS); 
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.0100)", "0", "Вход 1", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_1.LinkSetText_IndErr;
            ciIN_1.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            ciIN_1.OnChangeIndode += SynhrInd;
            arrVar.Add(efv);

            //// вход 2
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.0200)", "0", "Вход 2", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_2.LinkSetText_Ind;
            ciIN_2.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.0200)", "0", "Вход 2", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_2.LinkSetText_IndFlash;
            ciIN_2.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.0200)", "0", "Вход 2", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_2.LinkSetText_IndErr;
            ciIN_2.OnChangeIndode += SynhrInd;
            ciIN_2.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 3
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.0400)", "0", "Вход 3", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_3.LinkSetText_Ind;
            ciIN_3.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.0400)", "0", "Вход 3", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_3.LinkSetText_IndFlash;
            ciIN_3.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.0400)", "0", "Вход 3", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_3.LinkSetText_IndErr;
            ciIN_3.OnChangeIndode += SynhrInd;
            ciIN_3.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 4
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.0800)", "0", "Вход 4", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_4.LinkSetText_Ind;
            ciIN_4.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.0800)", "0", "Вход 4", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_4.LinkSetText_IndFlash;
            ciIN_4.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.0800)", "0", "Вход 4", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_4.LinkSetText_IndErr;
            ciIN_4.OnChangeIndode += SynhrInd;
            ciIN_4.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 5
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.1000)", "0", "Вход 5", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_5.LinkSetText_Ind;
            ciIN_5.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.1000)", "0", "Вход 5", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_5.LinkSetText_IndFlash;
            ciIN_5.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.1000)", "0", "Вход 5", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_5.LinkSetText_IndErr;
            ciIN_5.OnChangeIndode += SynhrInd;
            ciIN_5.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 6
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.2000)", "0", "Вход 6", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_6.LinkSetText_Ind;
            ciIN_6.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.2000)", "0", "Вход 6", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_6.LinkSetText_IndFlash;
            ciIN_6.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.2000)", "0", "Вход 6", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_6.LinkSetText_IndErr;
            ciIN_6.OnChangeIndode += SynhrInd;
            ciIN_6.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 7
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.4000)", "0", "Вход 7", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_7.LinkSetText_Ind;
            ciIN_7.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.4000)", "0", "Вход 7", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_7.LinkSetText_IndFlash;
            ciIN_7.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.4000)", "0", "Вход 7", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_7.LinkSetText_IndErr;
            ciIN_7.OnChangeIndode += SynhrInd;
            ciIN_7.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 8
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.8000)", "0", "Вход 8", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_8.LinkSetText_Ind;
            ciIN_8.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.8000)", "0", "Вход 8", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_8.LinkSetText_IndFlash;
            ciIN_8.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.8000)", "0", "Вход 8", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_8.LinkSetText_IndErr;
            ciIN_8.OnChangeIndode += SynhrInd;
            ciIN_8.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 9
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.0001)", "0", "Вход 9", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_9.LinkSetText_Ind;
            ciIN_9.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.0001)", "0", "Вход 9", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_9.LinkSetText_IndFlash;
            ciIN_9.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.0001)", "0", "Вход 9", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_9.LinkSetText_IndErr;
            ciIN_9.OnChangeIndode += SynhrInd;
            ciIN_9.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 10
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.0002)", "0", "Вход 10", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_10.LinkSetText_Ind;
            ciIN_10.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.0002)", "0", "Вход 10", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_10.LinkSetText_IndFlash;
            ciIN_10.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.0002)", "0", "Вход 10", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_10.LinkSetText_IndErr;
            ciIN_10.OnChangeIndode += SynhrInd;
            ciIN_10.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 11
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.0004)", "0", "Вход 11", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_11.LinkSetText_Ind;
            ciIN_11.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.0004)", "0", "Вход 11", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_11.LinkSetText_IndFlash;
            ciIN_11.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.0004)", "0", "Вход 11", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_11.LinkSetText_IndErr;
            ciIN_11.OnChangeIndode += SynhrInd;
            ciIN_11.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 12
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.0008)", "0", "Вход 12", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_12.LinkSetText_Ind;
            ciIN_12.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.0008)", "0", "Вход 12", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_12.LinkSetText_IndFlash;
            ciIN_12.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.0008)", "0", "Вход 12", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_12.LinkSetText_IndErr;
            ciIN_12.OnChangeIndode += SynhrInd;
            ciIN_12.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 13
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.0010)", "0", "Вход 13", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_13.LinkSetText_Ind;
            ciIN_13.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.0010)", "0", "Вход 13", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_13.LinkSetText_IndFlash;
            ciIN_13.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.0010)", "0", "Вход 13", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_13.LinkSetText_IndErr;
            ciIN_13.OnChangeIndode += SynhrInd;
            ciIN_13.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 14
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.0020)", "0", "Вход 14", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_14.LinkSetText_Ind;
            ciIN_14.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.0020)", "0", "Вход 14", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_14.LinkSetText_IndFlash;
            ciIN_14.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.0020)", "0", "Вход 14", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_14.LinkSetText_IndErr;
            ciIN_14.OnChangeIndode += SynhrInd;
            ciIN_14.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 15
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.0040)", "0", "Вход 15", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_15.LinkSetText_Ind;
            ciIN_15.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.0040)", "0", "Вход 15", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_15.LinkSetText_IndFlash;
            ciIN_15.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.0040)", "0", "Вход 15", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_15.LinkSetText_IndErr;
            ciIN_15.OnChangeIndode += SynhrInd;
            ciIN_15.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 16
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.112.0080)", "0", "Вход 16", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_16.LinkSetText_Ind;
            ciIN_16.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.115.0080)", "0", "Вход 16", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_16.LinkSetText_IndFlash;
            ciIN_16.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.118.0080)", "0", "Вход 16", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_16.LinkSetText_IndErr;
            ciIN_16.OnChangeIndode += SynhrInd;
            ciIN_16.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 17
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.0100)", "0", "Вход 17", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_17.LinkSetText_Ind;
            ciIN_17.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.0100)", "0", "Вход 17", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_17.LinkSetText_IndFlash;
            ciIN_17.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.0100)", "0", "Вход 17", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_17.LinkSetText_IndErr;
            ciIN_17.OnChangeIndode += SynhrInd;
            ciIN_17.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 18
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.0200)", "0", "Вход 18", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_18.LinkSetText_Ind;
            ciIN_18.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.0200)", "0", "Вход 18", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_18.LinkSetText_IndFlash;
            ciIN_18.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.0200)", "0", "Вход 18", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_18.LinkSetText_IndErr;
            ciIN_18.OnChangeIndode += SynhrInd;
            ciIN_18.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 19
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.0400)", "0", "Вход 19", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_19.LinkSetText_Ind;
            ciIN_19.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.0400)", "0", "Вход 19", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_19.LinkSetText_IndFlash;
            ciIN_19.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.0400)", "0", "Вход 19", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_19.LinkSetText_IndErr;
            ciIN_19.OnChangeIndode += SynhrInd;
            ciIN_19.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 20
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.0800)", "0", "Вход 20", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_20.LinkSetText_Ind;
            ciIN_20.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.0800)", "0", "Вход 20", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_20.LinkSetText_IndFlash;
            ciIN_20.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.0800)", "0", "Вход 20", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_20.LinkSetText_IndErr;
            ciIN_20.OnChangeIndode += SynhrInd;
            ciIN_20.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 21
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.1000)", "0", "Вход 21", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_21.LinkSetText_Ind;
            ciIN_21.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.1000)", "0", "Вход 21", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_21.LinkSetText_IndFlash;
            ciIN_21.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.1000)", "0", "Вход 21", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_21.LinkSetText_IndErr;
            ciIN_21.OnChangeIndode += SynhrInd;
            ciIN_21.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 22
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.2000)", "0", "Вход 22", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_22.LinkSetText_Ind;
            ciIN_22.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.2000)", "0", "Вход 22", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_22.LinkSetText_IndFlash;
            ciIN_22.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.2000)", "0", "Вход 22", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_22.LinkSetText_IndErr;
            ciIN_22.OnChangeIndode += SynhrInd;
            ciIN_22.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 23
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.4000)", "0", "Вход 23", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_23.LinkSetText_Ind;
            ciIN_23.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.4000)", "0", "Вход 23", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_23.LinkSetText_IndFlash;
            ciIN_23.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.4000)", "0", "Вход 23", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_23.LinkSetText_IndErr;
            ciIN_23.OnChangeIndode += SynhrInd;
            ciIN_23.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 24
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.8000)", "0", "Вход 24", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_24.LinkSetText_Ind;
            ciIN_24.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.8000)", "0", "Вход 24", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_24.LinkSetText_IndFlash;
            ciIN_24.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.8000)", "0", "Вход 24", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_24.LinkSetText_IndErr;
            ciIN_24.OnChangeIndode += SynhrInd;
            ciIN_24.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 25
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.0001)", "0", "Вход 25", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_25.LinkSetText_Ind;
            ciIN_25.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.0001)", "0", "Вход 25", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_25.LinkSetText_IndFlash;
            ciIN_25.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.0001)", "0", "Вход 25", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_25.LinkSetText_IndErr;
            ciIN_25.OnChangeIndode += SynhrInd;
            ciIN_25.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 26
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.0002)", "0", "Вход 26", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_26.LinkSetText_Ind;
            ciIN_26.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.0002)", "0", "Вход 26", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_26.LinkSetText_IndFlash;
            ciIN_26.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.0002)", "0", "Вход 26", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_26.LinkSetText_IndErr;
            ciIN_26.OnChangeIndode += SynhrInd;
            ciIN_26.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 27
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.0004)", "0", "Вход 27", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_27.LinkSetText_Ind;
            ciIN_27.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.0004)", "0", "Вход 27", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_27.LinkSetText_IndFlash;
            ciIN_27.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.0004)", "0", "Вход 27", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_27.LinkSetText_IndErr;
            ciIN_27.OnChangeIndode += SynhrInd;
            ciIN_27.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 28
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.0008)", "0", "Вход 28", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_28.LinkSetText_Ind;
            ciIN_28.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.0008)", "0", "Вход 28", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_28.LinkSetText_IndFlash;
            ciIN_28.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.0008)", "0", "Вход 28", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_28.LinkSetText_IndErr;
            ciIN_28.OnChangeIndode += SynhrInd;
            ciIN_28.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 29
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.0010)", "0", "Вход 29", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_29.LinkSetText_Ind;
            ciIN_29.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.0010)", "0", "Вход 29", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_29.LinkSetText_IndFlash;
            ciIN_29.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.0010)", "0", "Вход 29", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_29.LinkSetText_IndErr;
            ciIN_29.OnChangeIndode += SynhrInd;
            ciIN_29.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 30
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.0020)", "0", "Вход 30", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_30.LinkSetText_Ind;
            ciIN_30.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.0020)", "0", "Вход 30", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_30.LinkSetText_IndFlash;
            ciIN_30.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.0020)", "0", "Вход 30", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_30.LinkSetText_IndErr;
            ciIN_30.OnChangeIndode += SynhrInd;
            ciIN_30.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 31
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.0040)", "0", "Вход 31", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_31.LinkSetText_Ind;
            ciIN_31.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.0040)", "0", "Вход 31", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_31.LinkSetText_IndFlash;
            ciIN_31.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.0040)", "0", "Вход 31", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_31.LinkSetText_IndErr;
            ciIN_31.OnChangeIndode += SynhrInd;
            ciIN_31.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход 32
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.113.0080)", "0", "Вход 32", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_32.LinkSetText_Ind;
            ciIN_32.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.116.0080)", "0", "Вход 32", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_32.LinkSetText_IndFlash;
            ciIN_32.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.119.0080)", "0", "Вход 32", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciIN_32.LinkSetText_IndErr;
            ciIN_32.OnChangeIndode += SynhrInd;
            ciIN_32.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход КИС 1
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.114.0100)", "0", "Вход КИС 1", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciKIS_1.LinkSetText_Ind;
            ciKIS_1.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.117.0100)", "0", "Вход КИС 1", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciKIS_1.LinkSetText_IndFlash;
            ciKIS_1.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.120.0100)", "0", "Вход КИС 1", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciKIS_1.LinkSetText_IndErr;
            ciKIS_1.OnChangeIndode += SynhrInd;
            ciKIS_1.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход КИС 2
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.114.0200)", "0", "Вход КИС 2", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciKIS_2.LinkSetText_Ind;
            ciKIS_2.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.117.0200)", "0", "Вход КИС 2", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciKIS_2.LinkSetText_IndFlash;
            ciKIS_2.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.120.0200)", "0", "Вход КИС 2", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciKIS_2.LinkSetText_IndErr;
            ciKIS_2.OnChangeIndode += SynhrInd;
            ciKIS_2.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход КИС 3
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.114.0400)", "0", "Вход КИС 3", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciKIS_3.LinkSetText_Ind;
            ciKIS_3.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.117.0400)", "0", "Вход КИС 3", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciKIS_3.LinkSetText_IndFlash;
            ciKIS_3.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.120.0400)", "0", "Вход КИС 3", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciKIS_3.LinkSetText_IndErr;
            ciKIS_3.OnChangeIndode += SynhrInd;
            ciKIS_3.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);

            //// вход КИС 4
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.114.0800)", "0", "Вход КИС 4", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciKIS_4.LinkSetText_Ind;
            ciKIS_4.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.117.0800)", "0", "Вход КИС 4", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciKIS_4.LinkSetText_IndFlash;
            ciKIS_4.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
            efv = new FormulaEvalNDS(HMI_Settings.CONFIGURATION, "0(" + iFC + "." + iIDDev + ".5.120.0800)", "0", "Вход КИС 4", "", TypeOfTag.Discret, "");
            efv.OnChangeValFormTI += ciKIS_4.LinkSetText_IndErr;
            ciKIS_4.OnChangeIndode += SynhrInd;
            ciKIS_4.SetSubscribeLink2Tag(efv.LinkVariableNewDS);
            arrVar.Add(efv);
        ...
        private void btnAck_Click(object sender, EventArgs e)
        {
            ...
            /*ICommand cmd = */HMI_Settings.CONFIGURATION.ExecuteCommand(0, (uint)numdevfc, "ECC", new byte[] { }, this);

        }
		#endregion

        #region Вход на вкладку накопителя
        private void tbBMCS_Store_Enter()
        {
		...
		/// </summary>
        private void tbBMCS_Config_Enter()
		{
		    ...
		    TestCCforFLP(tabCntrl, slFLPUst);
      ...
      private void UstavBD()
		{
            ...
            int tbd = CommonUtils.CommonUtils.GetTypeBlockData4ThisDev(TypeBlockData4Req.TypeBlockData4Req_Ustavki, (uint)(iFC * 256 + iIDDev) );
