...
namespace HMI_MT
{
   public partial class frmSirius_SP : frmBMRZbase
   {
      #region Public
      ...
      OscDiagViewer oscdg;
      /// <summary>
      /// коллекция выделенных элементов в ListView 
      /// (для поддержания актуального текущего элемента списка)
      /// </summary>
      ...
      public frmSirius_SP( MainForm linkMainForm, int iFC, int iIDDev, string fxml )
      ...
      #region Load
      private void frmSirius_SP_Load( object sender, EventArgs e )
      {
		...
		ts = new TimeSpan(360, 0, 0, 0);
         ...
         // исходя из типа блока, убираем созданные по умолчанию, но незадействованные элементы
         DoExcludeInterfaceElements();

         lvsic = new ListView.SelectedListViewItemCollection(lstvAvar);
      }
      #endregion

      #region убираем созданные по умолчанию, но незадействованные элементы - для каждого типа блока отдельно
      private void DoExcludeInterfaceElements()
      {
         XDocument xdfrm = XDocument.Load(nfXMLConfig);
         string blockNamexml = xdfrm.Element("MT").Element("BMRZ").Element("frame").Attribute("Name").Value;
         if (blockNamexml.Contains("Sirius_RNM"))
            DoExcludeInterfaceElements_Sirius_RNM();
      }
      private void DoExcludeInterfaceElements_Sirius_RNM() 
      {
         //splitContainer7.Panel2Collapsed = true;

         foreach( TabPage tp in tc_Main.TabPages )
         {
            if (tp.Name == "tabPageOscDiag")
               tc_Main.TabPages.Remove(tp);
            else if (tp.Name == "tabpageSrabat")
               tc_Main.TabPages.Remove(tp);
      ...
      void tabpageControl_Enter( object sender, EventArgs e )
      {
         ...
         PrepareTabPagesForGroup( tp_this.Text, tp_this, ref arrVars, pnlTPControl );
      ...
      private void DiagBD()
      {
         ...
         oscdg.DTEndData = pnlOscDiag.dtpEndData.Value;
         oscdg.DTEndTime = pnlOscDiag.dtpEndData.Value;
         oscdg.TypeRec = CommonUtils.CommonUtils.GetTypeBlockData4ThisDev(TypeBlockData4Req.TypeBlockData4Req_Diagramm,parent.KB, this.IFC, this.IIDDev);// TypeBlockData.TypeBlockData_Diagramm;
      ...
      #region объединяем осциллограммы
      #region старый код
      //private void button4_Click_Local(object sender, EventArgs e)
      //{
      ...
      //         asb.Add(AppDomain.CurrentDomain.BaseDirectory + "\\Oscillogramms\\" + sb);
      //         ifa = AppDomain.CurrentDomain.BaseDirectory + "\\Oscillogramms\\" + sb.ToString();

      //         FileStream f = File.Create(AppDomain.CurrentDomain.BaseDirectory + "\\Oscillogramms\\" + sb.ToString());
      //         f.Write(arrO, 0, arrO.Length);
      //         f.Close();
      //      }
      //   }
      ...
      //   }
      //   prc.StartInfo.FileName = AppDomain.CurrentDomain.BaseDirectory + "\\OscView\\OscView.exe  ";
      //   //prc.StartInfo.Arguments = "-t " + "\"" + sb.ToString() + "\"";
      //   prc.StartInfo.Arguments = "-t " + sb.ToString();
      //   prc.Start();
      //}       
      #endregion
      ...
      private void LogDeviceBD()
      {
         ...
         SqlCommand cmd = new SqlCommand("GetEventBlock", asqlconnect);
         ...
         ptypeRec.Value = CommonUtils.CommonUtils.GetTypeBlockData4ThisDev(TypeBlockData4Req.TypeBlockData_LogEventBlock, parent.KB, this.IFC, this.IIDDev);// TypeBlockData.TypeBlockData_LogEventBlock; // информация по журналу событий блока
         ...
         // заполняем ListView через сортированный список
         SortedList<DateTime, ListViewItem> slRecEvent = new SortedList<DateTime, ListViewItem>();
         ...
         for (int curRow = 0; curRow < dtLD.Rows.Count; curRow++)
         {
            ...
            DateTime dtfcread = new DateTime(2000 + Convert.ToInt32(adata[6]), Convert.ToInt32(adata[5]),Convert.ToInt32(adata[4]),Convert.ToInt32(adata[3]),Convert.ToInt32(adata[2]),Convert.ToInt32(adata[1]),Convert.ToInt32(sec100));
            ...
            while (posMain < adata.Length)
            {
               ...
               double tdInMSec10 = Convert.ToDouble(tInMSec10);
               ...
               dtcuryear = dtcuryear.AddMilliseconds(Convert.ToDouble(tdInMSec10 * 10));
               pos += 4;

               if (dtcuryear > dtfcread)
                  dtcuryear = dtcuryear.AddYears(-1);
               ...
               ListViewItem li = new ListViewItem(dtcuryear.ToLongDateString() + " : " + dtcuryear.ToLongTimeString() + " : " + dtcuryear.Millisecond.ToString());
               ...
               DateTime keysl = dtcuryear.AddMilliseconds( keyadvcnt );

               while( slRecEvent.ContainsKey( keysl ) )
               {
                  keyadvcnt++;   // индекс для уникальности записи
                  keysl = dtcuryear.AddMilliseconds( keyadvcnt );
               }
               slRecEvent.Add( keysl, li );

               //slRecEvent.Add(dtcuryear, li);
            }
         }

         //foreach( ListViewItem liv in slRecEvent.Values )
         for (int i = slRecEvent.Values.Count; i > 0; i-- )
            lstvEventLog.Items.Add(slRecEvent.Values[i-1]);
      ...
      private void lstvAvar_ItemActivate( object sender, EventArgs e )
      {
         ...
         // получение строк соединения и поставщика данных из файла *.config
            ...
            ParseBDPacket( adata, GetAdrBlockData(path2DeviceCFG,8), 8 ); // или 61100 ???
      ...
      private void AvarBD( )
      {
         ...
         ptypeRec.ParameterName = "@type";
         ptypeRec.SqlDbType = SqlDbType.Int;
         ptypeRec.Value = CommonUtils.CommonUtils.GetTypeBlockData4ThisDev(TypeBlockData4Req.TypeBlockData4Req_Srabat, parent.KB, this.IFC, this.IIDDev);// TypeBlockData.TypeBlockData_Srabat;
         ptypeRec.Direction = ParameterDirection.Input;
         cmd.Parameters.Add( ptypeRec );
         // 6. ид записи журнала
         SqlParameter pid = new SqlParameter( );
         ...
         for ( int curRow = 0 ;curRow < dtA.Rows.Count ;curRow++ )
         {
            ...
            ListViewItem li = new ListViewItem( CRZADevices.CommonCRZADeviceFunction.GetTimeInMTRACustomFormat( t ) );//t.ToShortDateString( )
      ...
      private void UstavBD( )
   {
      ...
      ptypeRec.ParameterName = "@type";
      ptypeRec.SqlDbType = SqlDbType.Int;
      ptypeRec.Value = CommonUtils.CommonUtils.GetTypeBlockData4ThisDev(TypeBlockData4Req.TypeBlockData4Req_Ustavki, parent.KB, this.IFC, this.IIDDev);// TypeBlockData.TypeBlockData_Ustavki;
      ptypeRec.Direction = ParameterDirection.Input;
      cmd.Parameters.Add( ptypeRec );
      // 6. ид записи журнала
      SqlParameter pid = new SqlParameter( );
      ...
      private void lstvEventLog_SelectedIndexChanged( object sender, EventArgs e )
      {
